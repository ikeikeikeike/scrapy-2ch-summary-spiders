...
class MashlifeSpider(ThreadFloatBbsSpider):
    """ for mashlife.doorblog.jp
    """
    name = 'mashlife'
    allowed_domains = ['mashlife.doorblog.jp']
    start_urls = ['http://mashlife.doorblog.jp/index.rdf']
    ...
    def extract_source(self, selector):
        ...
        try:
            url = [
                text for
                text in selector.css('div.article-body-inner span').xpath('text()').extract()
    ...
    def extract_tags(self, selector, response):
        ...
        try:
            ...
            tags = [self.get_text(tag) for tag in selector.css('.article-header-category dd a')]
